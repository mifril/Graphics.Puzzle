    ...
    public static void useBilinearFilter(BufferedImage image) {
        ...
        for (int y = 1; y < height - 1; ++y) {
            for (int x = 1; x < width - 1; ++x) {
                ...
                int alpha = (int) (colorNW.getAlpha() * factor1 + colorNE.getAlpha() * factor2 + colorSW.getAlpha() * factor3 + colorSE.getAlpha() * factor4);
...
*/
    ...
    public static int getBilinearFilteredPixel(BufferedImage image, int x, int y, boolean needFiltering) {
        int width = image.getWidth();
        int height = image.getHeight();
        ...
        double factor3 = (1 - 0.5) * 0.5;
        double factor4 = 0.5 * 0.5;
        int pixelNW = image.getRGB(x - 1, y - 1);
        int pixelNE = image.getRGB(x + 1, y - 1);
        int pixelSW = image.getRGB(x - 1, y + 1);
        int pixelSE = image.getRGB(x + 1, y + 1);
        Color colorNW = new Color(pixelNW);
        Color colorNE = new Color(pixelNE);
        Color colorSW = new Color(pixelSW);
        Color colorSE = new Color(pixelSE);
